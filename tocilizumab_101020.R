...
# confirm the same EMPIs are kept
sum(biologicsa_first$EMPI %in% biologics_a$EMPI) #1097
...
# confirm the same EMPIs are kept
sum(dmard_first_a$EMPI %in% a_first$EMPI) #628
...
sum(dmard_first_b$EMPI %in% b_first$EMPI) #521
...
sum(dmard_first_ab$EMPI %in% ab_first$EMPI) #1008 
##
##
##
# 4. Encounters
...
outpt_enc_rheum$Principal_Diagnosis[outpt_enc_rheum$Principal_Diagnosis == ""] <- NA
...
diagnosis_rheum <- outpt_rheum_enc_DX %>%
  group_by(EMPI) %>%
  mutate(dx_mode = Mode(Principal_Diagnosis))
...
lab_SAg_first <- lab_SAg[order(lab_SAg$EMPI, mdy_hm(lab_SAg$Lab_Date), decreasing = FALSE),]
