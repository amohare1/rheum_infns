...
biol_ab <- grepl(paste("Certolizumab|Cimzia|Etanercept|Enbrel|Adalimumab|Humira|Anakinra|Kineret|",
              ...
              med_in$Medication,ignore.case=T)
non_biol <- grepl(paste("Hydroxychloroquine|Plaquenil|Leflunomide|Arava|Methotrexate|Otrexup|Xatmep|Trexall|Rasuvo", ##Serena: lost | at the end
                        "Sulfasalazine|Azulfidine|Azathioprine|Imuran|Azasan|Mycophenolate|Myfortic|Cellcept",    ##Serena: lost | at the end
                        "Cyclosporine|NeoralGengraf|Sandimmune",sep=""),med_in$Medication,ignore.case=T) ##### Serena: non_biol has 22004 true ####
...
length(unique(dmard$EMPI)) #1034 unique EMPIs
...
# confirm the same EMPIs are kept
sum(dmard_first$EMPI %in% dmard$EMPI) #1034
...
# confirm the same EMPIs are kept
sum(dmard_first_a$EMPI %in% a_first$EMPI) #628 ### Serena: 637 ###
#This means that 628 of the drug_a patients also received a non-biologic DMARD
...
sum(dmard_first_b$EMPI %in% b_first$EMPI) #521 #### Serena: 530;
#521/627 of the drug_b patients also received a non-biologic DMARD
...
sum(dmard_first_ab$EMPI %in% ab_first$EMPI) #1008 #Serena: 1026
##
##
##
# 4. Encounters
...
# Remove the missing values from Principal Diagnosis.
outpt_enc_rheum$Principle_Diagnosis[outpt_enc_rheum$Principle_Diagnosis == ""] <- NA #### Serena: typo, change Principal_Diagnosis -> Principle_Diagnosis
outpt_rheum_enc_DX <- outpt_enc_rheum[!is.na(outpt_enc_rheum$Principle_Diagnosis),] 

# Pass  Mode function to the Principal_Diagnosis for each EMPI -- create a new array called diagnosis_rheum
diagnosis_rheum <- outpt_enc_rheum %>%
  group_by(EMPI) %>%
  mutate(dx_mode = Mode(Principle_Diagnosis)) #### Serena: typo, change Principal_Diagnosis -> Principle_Diagnosis
### Serena: I guess that you want to grab the most freq diagnosis for each empi, and add it to dx_mode

# Create a column which has the mode Principal Diagnosis from all the rheum encounters for each EMPI
...
diagnosis_list_a <- a_included %>% 
  ...
  tally()
# Write to a csv file
write.csv(diagnosis_list_a, "toci_diagnosis_101020.csv")
#Same for B
diagnosis_list_b <- b_included %>% 
  group_by(dx_mode) %>% 
  tally()
# Write to a csv file
write.csv(diagnosis_list_b, "tofa_diagnosis_101020.csv")
##
##
##
# 3. Lab Data
...
# Keep only the first HBV SAg test date
lab_SAb_first <- lab_SAb[order(lab_SAb$EMPI, mdy_hm(lab_SAb$Lab_Date), decreasing = FALSE),]
...
lab_CAb_first <- lab_CAb[order(lab_CAb$EMPI, mdy_hm(lab_CAb$Lab_Date), decreasing = FALSE),]
...
# Keep only the first HBV CAb test date
lab_CAbigm_first <- lab_CAbigm[order(lab_CAbigm$EMPI, mdy_hm(lab_CAbigm$Lab_Date), decreasing = FALSE),]
...
lab_HBVeag_first <- lab_HBVeag[order(lab_HBVeag$EMPI, mdy_hm(lab_HBVeag$Lab_Date), decreasing = FALSE),]
lab_HBVeag_first <- lab_HBVeag_first[-which(duplicated(lab_HBVeag_first$EMPI)),]
# Keep only HBV e-Ab
lab_HBVeab <- labs[labs$Lab_test == "HBV e Ab",]
lab_HBVeab_first <- lab_HBVeab[order(lab_HBVeab$EMPI, mdy_hm(lab_HBVeab$Lab_Date), decreasing = FALSE),]
lab_HBVeab_first <- lab_HBVeab_first[-which(duplicated(lab_HBVeab_first$EMPI)),]
# Keep only HBV DNA
lab_HBVdna <- labs[labs$Lab_test == "HBV DNA (IU/mL)",]
lab_HBVdna_first <- lab_HBVdna[order(lab_HBVdna$EMPI, mdy_hm(lab_HBVdna$Lab_Date), decreasing = FALSE),]
